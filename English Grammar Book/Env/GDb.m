...
- (double)getFloat:(NSString*)columnName {
    ...
    NSNumber *col = _columnMap[columnName];
    if(col == nil) return 0;
    return sqlite3_column_double(_stmt, col.floatValue);
}
...
{
    int rc;
    NSString *path = [GFileUtils documentPath: @"gdata.db"];
    sqlite3 *db;
    
    NSInteger version = [[NSUserDefaults standardUserDefaults] integerForKey: @"gdb_version"];
    ...
    [[NSUserDefaults standardUserDefaults] setInteger:DB_VERSION forKey:@"gdb_version"];
...
- (void) copyDb
{
    ...
    NSError *error;
    NSString *dbSrc = [GFileUtils resourcePath: @"gdata.db"];
    NSString *dbDst =  [GFileUtils documentPath: @"gdata.db"];
    ...
    if (success) {
        ...
        [[NSURL fileURLWithPath:dbDst] setResourceValue:[NSNumber numberWithBool:YES] forKey:NSURLIsExcludedFromBackupKey error:nil];
    }
}
...
- (GCursor*)prepareCursor:(NSString*)query;
{
    ...
    if(sqlite3_prepare_v2(_datadb, command, -1, &pStmt, NULL) == SQLITE_OK) {
        return [[GCursor alloc] initWithStmt:pStmt];
    }
    return NULL;
}
