...
NSString *const IAPHelperProductPurchasedNotification1 = @"IAPHelperProductPurchasedNotification";

@interface GInAppPurchaseController()
...
@implementation GInAppPurchaseController
...
+ (GInAppPurchaseController*)sharedController {
    static GInAppPurchaseController* sController = nil;
    if (sController == nil) {
        sController = [GInAppPurchaseController new];
    }
    return sController;
}
...
- (id)init {
    self = [super init];
    if (self) {
        productIdentifierList = @[kIAPItemRemoveAds, kIAPItemDonate, kIAPItemRemoveAdsAndDonate];
...
- (void)provideContentForProductIdentifier:(NSString *)productIdentifier restore:(BOOL)restore {
    ...
    ////
    [[NSNotificationCenter defaultCenter] postNotificationName:IAPHelperProductPurchasedNotification1 object:nil userInfo:@{IAPHelperProductPurchasedNotification1: productIdentifier}];
}
