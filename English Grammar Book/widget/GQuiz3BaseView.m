...
- (void) addQuizWordView:(NSString *)text rt:(CGRect) rect
{
    ...
    wordView.textColor = [UIColor blackColor];
    wordView.backgroundColor = [UIColor clearColor];
    wordView.font = [UIFont systemFontOfSize:16.0f];
    wordView.adjustsFontSizeToFitWidth = YES;
...
- (UILabel *) addChoiceWordView:(NSString *)text rt:(CGRect) rect
{
    ...
    wordView.layer.cornerRadius = 5; // this value vary as per your desire
...
- (void) initEveryCellLength
{
    ...
    float choiceWordWith = [self getChoiceWordWidth];
    
    [self.aryCellWidths addObject:[NSNumber numberWithFloat:choiceWordWith]];
    ...
    for (int index = 0 ; index < words.count ; index ++) {
        ...
        float wordWidth = [self getTextWidth:word size:16.0f];
...
- (NSArray *) getQuizWords
{
    ...
    quiz = [quiz stringByReplacingOccurrencesOfString:@"." withString:@" ."];
...
- (NSArray *) getChoices
{
    ...
    choice = [self trimAndRemoveWhiteSpaceAndLinebreack:choice];
...
- (NSInteger) checkAnswer
{
    ...
    if([answer containsString:correctAnswer]) {
...
- (NSString *) getUserAnswer
{
    ...
    for (int index = 0 ; index < self.aryCellWidths.count ; index ++) {
        ...
        if(isWord == 0) {
            ...
            userAnswer = [NSString stringWithFormat:@"%@ %@", userAnswer, word];
        }
    }
