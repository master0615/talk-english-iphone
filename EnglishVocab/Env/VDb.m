...
{
    int rc;
    NSString *path = [VFileUtils documentPath: @"vdata.db"];
    sqlite3 *db;
    
    NSInteger version = [[NSUserDefaults standardUserDefaults] integerForKey: @"vdb_version"];
    ...
    [[NSUserDefaults standardUserDefaults] setInteger:DB_VERSION forKey:@"vdb_version"];
...
- (void) copyDb
{
    ...
    NSError *error;
    NSString *dbSrc = [VFileUtils resourcePath: @"vdata.db"];
    NSString *dbDst =  [VFileUtils documentPath: @"vdata.db"];
...
- (VCursor*)prepareCursor:(NSString*)query;
{
    ...
    if(sqlite3_prepare_v2(_datadb, command, -1, &pStmt, NULL) == SQLITE_OK) {
        return [[VCursor alloc] initWithStmt:pStmt];
    }
    return NULL;
}
