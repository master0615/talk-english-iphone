...
CGFloat const PMBProgressMaxOffset = 1000000.f;
...
@interface PMBProgressHUDRoundedButton : UIButton
@end


@implementation PMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    PMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    PMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (PMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (PMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)done {
    ...
    id<PMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    PMBBackgroundView *backgroundView = [[PMBBackgroundView alloc] initWithFrame:self.bounds];
    ...
    PMBBackgroundView *bezelView = [PMBBackgroundView new];
    bezelView.translatesAutoresizingMaskIntoConstraints = NO;
    bezelView.layer.cornerRadius = 5.f;
    bezelView.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed:192.0f/255.0f green:217.0f/255.0f blue:239.0f/255.0f alpha:1.0f];
    bezelView.alpha = 0.9f;
    ...
    UIButton *button = [PMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[PMBRoundProgressView class]];
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[PMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[PMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MBProgressHUDModeAnnularDeterminate) {
            [(PMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
...
#pragma clang diagnostic pop
    ...
    } else if ([indicator isKindOfClass:[PMBRoundProgressView class]]) {
        ((PMBRoundProgressView *)indicator).progressTintColor = color;
        ((PMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
    } else if ([indicator isKindOfClass:[PMBBarProgressView class]]) {
        ((PMBBarProgressView *)indicator).progressColor = color;
        ((PMBBarProgressView *)indicator).lineColor = color;
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
    PMBBackgroundView *bezelView = self.bezelView;
...
@implementation PMBRoundProgressView
...
@implementation PMBBarProgressView
...
@implementation PMBBackgroundView
...
@implementation PMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [PMBProgressHUD allHUDsForView:view];
    for (PMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (BOOL)dimBackground {
    PMBBackgroundView *backgroundView = self.backgroundView;
...
@implementation PMBProgressHUDRoundedButton
