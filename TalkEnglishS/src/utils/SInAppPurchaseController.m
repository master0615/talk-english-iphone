...
NSString *const IAPHelperProductPurchasedNotifications = @"IAPHelperProductPurchasedNotification";


@interface SInAppPurchaseController()
...
@implementation SInAppPurchaseController
...
+ (SInAppPurchaseController*)sharedController {
    static SInAppPurchaseController* sController = nil;
    if (sController == nil) {
        sController = [SInAppPurchaseController new];
        sController.isPurchased = [[NSUserDefaults standardUserDefaults] integerForKey:@"isPurchased"];
    }
    return sController;
}
...
- (id)init {
    self = [super init];
    if (self) {
        productIdentifierList = @[kIAPItemProductId299, kIAPItemProductId499];
...
- (void)provideContentForProductIdentifier:(NSString *)productIdentifier restore:(BOOL)restore {
    ...
    ////
    [[NSNotificationCenter defaultCenter] postNotificationName:IAPHelperProductPurchasedNotifications object:nil userInfo:@{IAPHelperProductPurchasedNotifications: productIdentifier}];
}
