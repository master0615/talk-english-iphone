...
{
    int rc;
    NSString *path = [FileUtils documentPath: @"bdata.jpg"];
    sqlite3 *db;
    
    NSInteger version = [[NSUserDefaults standardUserDefaults] integerForKey: @"bdb_version"];
    ...
    [[NSUserDefaults standardUserDefaults] setInteger:DB_VERSION forKey:@"bdb_version"];
...
- (void) copyDb
{
    ...
    NSError *error;
    NSString *dbSrc = [FileUtils resourcePath: @"bdata.jpg"];
    NSString *dbDst =  [FileUtils documentPath: @"bdata.jpg"];
...
- (BCursor*)prepareCursor:(NSString*)query;
{
    ...
    if(sqlite3_prepare_v2(_datadb, command, -1, &pStmt, NULL) == SQLITE_OK) {
        return [[BCursor alloc] initWithStmt:pStmt];
    }
    return NULL;
}
