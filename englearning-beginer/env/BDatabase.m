...
@interface BDatabase ()
@property (strong, nonatomic) BSQLiteManager *dbManager;
@end


@implementation BDatabase


+(BDatabase*)sharedInstance{
    static BDatabase* db = nil;
    if (db == nil) {
        db = [[BDatabase alloc] init];
    }
    return db;
}
- (instancetype)init{
...
- (void)openDB {
    ...
    if (dbPath) {
        self.dbManager = [[BSQLiteManager alloc] initWithDatabaseNamed:dbPath];
    }
    if (self.dbManager) {
