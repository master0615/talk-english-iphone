...
#import "BSQLiteManager.h"

// Private methods
@interface BSQLiteManager (Private)

- (NSString *)getDatabasePath;
- (NSError *)createDBErrorWithDescription:(NSString*)description andCode:(NSInteger)code;

@end



@implementation BSQLiteManager
 ...
 *
 * @return the BSQLiteManager object initialised.
 */
...
- (NSError *) openDatabase {
	...
	NSInteger result = sqlite3_open(dbpath, &db);
	if (result != SQLITE_OK) {
...
- (NSError *)doQuery:(NSString *)sql {
	...
	if (openError == nil) {
		...
		NSInteger nError = sqlite3_prepare_v2(db, query, -1, &statement, NULL);
...
- (NSArray *)getRowsForQuery:(NSString *)sql {
	...
	sqlite3_stmt *statement;
	const char *query = [sql UTF8String];
	NSInteger returnCode = sqlite3_prepare_v2(db, query, -1, &statement, NULL);
	...
	while (sqlite3_step(statement) == SQLITE_ROW) {
		...
		for (int i = 0; i<columns; i++) {
			...
			NSInteger type = sqlite3_column_type(statement, i);
			
			switch (type) {
				case SQLITE_INTEGER:
				{
					NSInteger value = sqlite3_column_int(statement, i);
					[result setObject:[NSNumber numberWithInteger:value] forKey:columnName];
					break;
				}
				case SQLITE_FLOAT:
				{
...
- (NSString *)getDatabaseDump {
	...
	[dump appendString:@";\n; Dump generated with BSQLiteManager4iOS \n;\n; By Misato (2011)\n"];
	...
	for (NSInteger i = 0; i<[rows count]; i++) {
		...
		for (NSInteger j = 0; j<[tableContent count]; j++) {
...
#pragma mark -
@implementation BSQLiteManager (Private)
...
- (NSString *)getDatabasePath {
	if(![[NSFileManager defaultManager] fileExistsAtPath:databaseName]){
        ...
        const char *dbpath = [databaseName UTF8String];
        
        if (sqlite3_open(dbpath, &db) == SQLITE_OK) {
            char *errMsg;
            const char *sql_stmt = "CREATE TABLE IF NOT EXISTS BOOKMARK (ID INTEGER PRIMARY KEY AUTOINCREMENT, MAIN TEXT, SUB TEXT, TITLE TEXT)";
            
            if (sqlite3_exec(db, sql_stmt, NULL, NULL, &errMsg) != SQLITE_OK) {
                NSLog(@"FAILED TO CREATE Table1");
            }
            
            sql_stmt = "CREATE TABLE IF NOT EXISTS RECORD (ID INTEGER PRIMARY KEY AUTOINCREMENT, TITLE TEXT, LESSON TEXT, TIME TEXT)";
