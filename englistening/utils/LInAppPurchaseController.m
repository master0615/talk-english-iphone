...
@interface LInAppPurchaseController()
...
@implementation LInAppPurchaseController
...
+ (LInAppPurchaseController*)sharedController {
    static LInAppPurchaseController* sController = nil;
    if (sController == nil) {
        sController = [LInAppPurchaseController new];
    }
    return sController;
}
...
- (void)provideContentForProductIdentifier:(NSString *)productIdentifier restore:(BOOL)restore {
    ...
    ////
    [[NSNotificationCenter defaultCenter] postNotificationName:IAPHelperProductPurchasedNotification2 object:nil userInfo:@{IAPHelperProductPurchasedNotification2: productIdentifier}];
}
